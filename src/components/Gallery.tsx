      ...
      <div className="max-w-[1800px] mx-auto">
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 px-4 md:px-8 lg:px-12 max-w-7xl mx-auto">
            {beforeAfterComparisons.map((comparison, index) => <div key={index} className="flex flex-col">
                <p className="text-base font-medium text-muted-foreground mb-3 text-center">
                  {comparison.title}
                </p>
                <BeforeAfterSlider beforeImage={comparison.before} afterImage={comparison.after} beforeAlt={comparison.beforeAlt} afterAlt={comparison.afterAlt} aspectRatio="5/7" />
              </div>)}
          </div>
        </div>
        ...
        {/* Video Gallery */}
        <div className="px-4">
          <h3 className="text-2xl md:text-3xl font-bold mb-8 text-foreground text-center">Finished Bathroom Videos</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 max-w-7xl mx-auto">
            {videos.map((item, index) => <div key={index} className="group relative aspect-video overflow-hidden rounded-2xl cursor-pointer shadow-soft hover:shadow-elegant transition-all duration-300" onClick={() => setSelectedVideo(item.video)}>
                <img src={item.thumbnail} alt={item.description} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110" />
